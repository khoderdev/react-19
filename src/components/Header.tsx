...
export default function Header() {
  ...
  const handleLogin = () => {
    const foundUser = Users.find(
      (user) => user.username === "admin" && user.password === "admin123"
    );
    if (foundUser) {
      setIsLoggedIn(true);
  ...
  //   const [visible, setVisible] = useState(true);
  //   const [, setUserName] = useAtom(userNameAtom);
  //   const [, setUserRules] = useAtom(userRolesAtom);
  //   const handleScroll = () => {
  //     const currentScrollPos = window.pageYOffset;
  //     setVisible(
  //       (prevScrollPos > currentScrollPos &&
  ...
  //   };
    ...
    const handleLogout = () => {
      ...
      setUserRules(''); // Reset user rules
  };

  return (
    <>
      <header
        className={`h-14  text-black-text dark:text-white-text fixed top-0 border-b border-b-white-contents dark:border-b-black-contents !drop-shadow-lg w-full transition-all duration-300 flex justify-center ${
          visible ? "" : ""
        }`}
        style={{
      ...
      >
        <nav
          className="mx-auto flex items-center justify-between p-4 py-2 lg:px- max-w-screen-xl w-full"
          aria-label="Global"
        >
          <Link to="/" className="-m-1.5 p-1.5">
            <div className="!text-[2.5rem] font-bold text-[#ef4444]">
              React 19
            </div>
          </Link>
          ...
          {isLoggedIn ? (
            <button
              className="hidden lg:block text-md font-semibold leading-6 "
